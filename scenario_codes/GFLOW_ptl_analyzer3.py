...
#
# To run:  python GFLOW_ptl_analyzer3.py namefile.nam
# ##############################################################################
...
if method == 1:
	try:
		...
		acroutshapename = acroutfilename[:-4] + '.shp'
	except:
...
if (method == 2 or method == 4 ):
	try:	
		...
		eleoutshapename = eleoutfilename[:-4] + '.shp'
	except:
...
if ((method == 2 or method == 4) and ((allin['welloutfilename'].lower() <> 'none'))):  # Well elements are points and need a separate output shapefile
	try:	
		...
		welloutshapename = welloutfilename[:-4] + '.shp'
	except:
...
if method >=3: 
	try:
		...
		gridoutshapename = gridoutfilename[:-4] + '.shp'
	except:
		raise(FileFail(gridoutfilename,'gridoutput file'))	

#open method dependent input files
if method == 1:
	try:
...
if method == 1:
	if shapefiles_imported == True:
		...
		pshape = shapefile.Writer(acroutshapename, shapefile.POINT)        #set up the point shapefile
		pshape.field('In_Out')
	else: pshape = -999
...
if (method == 2 or method == 4): 
	if shapefiles_imported == True:
		...
		lshape = shapefile.Writer(eleoutshapename, shapefile.POLYLINE)        #set up the line output shapefile
	...
	if len(wells) > 0:
		if shapefiles_imported == True:		
			#pshape = shapefile.Writer(shapefile.POINT)        #set up the point output shapefile
			pshape = shapefile.Writer(welloutshapename, shapefile.POINT)
			pshape.field('In_Out')	
		else: pshape = -999
...
if (method >= 3):	
	if shapefiles_imported == True:	
		...
		nodeshape = shapefile.Writer(gridoutshapename, shapefile.POINT)
